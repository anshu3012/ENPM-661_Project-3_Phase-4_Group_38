...
print("This code will implement A* Algorithm for a Rigid Robot with Non-Holonomic Constraints")
...
plt.xlim(xMin,xMax)
plt.ylim(yMin,yMax)
plt.title('ENPM661 Project-3 Phase-3 Group-38',fontsize=10)
...
for centre in centres:
    ax.add_artist(plt.Circle(centre, cirR, color = "red"))
# squares
corners = [(-4.85, -0.75), (-2.85, 2.35), (3.35, -0.75)]
for corner in corners:
    ax.add_artist(plt.Rectangle((corner[0] - clr, corner[1] - clr), 1.5 + 2 * clr, 1.5 + 2 * clr, fc='r'))
# border
rc = clr + 0.1 # border of map
borderTop = [[xMax,yMax],[xMax,yMax - rc],[xMin,yMax - rc],[xMin,xMax]] # top border
borderRight = [[xMax,yMax],[xMax,yMin],[xMax - rc,yMin],[xMax - rc,yMax]] # right border
borderBottom = [[xMax,yMin + rc],[xMax,yMin],[xMin,yMin],[xMin,yMin + rc]] # bottom border
borderLeft = [[xMin + rc,yMax],[xMin + rc,yMin],[xMin,yMin],[xMin,yMax]] # left border
...
# Plot curve
def plot_curve(uL,uR,point,color):
    """
...
def a_star_algo(clr, start_node_pos, goal_node_pos):
    ...
    # Show goal region
    goal_region = 0.3
    ax.add_artist(plt.Circle((goal_cords_x, goal_cords_y), goal_region, color = "green"))
    # A star search
    while queue:
        ...
        for action in actions:
            ...
            if in_obstacle != 1 and path_in_obstacle == False:
                ...
                if visited[int(new_point[0]/thresholdXY)][int(new_point[1]/thresholdXY)][int(new_point[2]/thresholdTheta)] == 0:
                    ...
                    counter += 1
                    plot_curve(w1,w2,current_point,"black")
...
if result is not None:
    ...
    for elem in nodes_list:
        ...
        w1.insert(0,elem.action[0])
        w2.insert(0,elem.action[1])
    for index in range(1,len(x)):
        if index != (len(x) - 2):
            ...
            W1 = w1[index]
            W2 = w2[index]
            #print("x ", X, "y ", Y, "theta ", Theta, "w1 ", W1, "w2 ", W2)
            plot_curve(W1,W2,[X,Y,Theta],"blue")
